...
function CaptureHtml($htmlName, $pngName, $port) {
    $htmlPath = "d:\Project\managment Softwares\madrsa abdul rehman bin auf\scratch\$htmlName"
    $url = 'file:///' + $htmlPath.Replace('\', '/')
    $outPath = "d:\Project\managment Softwares\madrsa abdul rehman bin auf\scratch\$pngName"
    ...
    try {
        ...
        if ($pageTarget) {
            ...
            if ($jsonResult.result -and $jsonResult.result.data) {
                ...
                Write-Output "SUCCESS: Saved $outPath"
            }
...
CaptureHtml "var1.html" "var1.png" 9224
CaptureHtml "var2.html" "var2.png" 9225
