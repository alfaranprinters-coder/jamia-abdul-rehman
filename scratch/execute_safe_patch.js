...
const marker1 = `<div style="font-size: 1.1rem; font-weight: bold; color: #059669; font-family: monospace;">\${student.uniqueCode || ('STU-' + (1000 + parseInt(student.id)))}</div>`;
const marker2 = `<th>رابطہ</th>`;
...
const endOfMarker1 = idx1 + marker1.length;
const idx2 = content.indexOf(marker2, endOfMarker1);
if (idx2 === -1) {
    console.error('marker2 not found after marker1!');
    process.exit(1);
}

console.log('Found cut between', endOfMarker1, 'and', idx2);
console.log('Text between (should be whitespace/newlines):', JSON.stringify(content.substring(endOfMarker1, idx2)));
...
const newContent = content.substring(0, endOfMarker1) + insertion + content.substring(idx2);

console.log('Testing new content with vm.Script...');
try {
    new vm.Script(newContent);
    console.log('SUCCESS: vm.Script passed syntax check with zero errors!');
} catch (e) {
    console.error('SYNTAX ERROR:', e);
    process.exit(1);
}

fs.writeFileSync(appPath + '.bak_before_safe_patch', content, 'utf8');
